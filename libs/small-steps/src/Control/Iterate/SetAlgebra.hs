...
-- =========================================================================
-- The basic strategy is for programers to write set algebra expressions,
-- wrap them in an 'eval' and then let the system decide how to efficently execute
-- the expression. An example: eval(dom (delegs ▷ Set.singleton hk) ◁ stake)
-- The strategy is to use class magic (Embed, HasExp, HasQuery from Control.Iterate.BaseTypes)
-- to translate (dom (delegs ▷ Set.singleton hk) ◁ stake) into the Exp datatype which is
-- deep embedding of the set algebra (see module Control.Iterate.Exp). We then attempt to
-- apply rewrite rules (compute :: Exp t -> t, from this module). If there are no applicable
-- rewrite rules, we use (compile :: Exp (f k v) -> (Query k v, BaseRep f k v)) from this module
-- to transate the Exp into Query, and a target type (BaseRep f k v), and a generic but slower
-- mechanism (run :: (Ord k) => (Query k v, BaseRep f k v) -> f k v) is used to get the result.

-- =========================================================================
-- Every Exp can be compiled into a Query. This is one part of the
-- slower, generic agorithm, that provides an implementation for every
-- well-typed Exp.
