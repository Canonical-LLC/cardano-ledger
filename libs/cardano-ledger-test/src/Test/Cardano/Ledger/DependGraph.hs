...
import Cardano.Ledger.BaseTypes (Globals (..), UnitInterval, boundRational, epochInfo)
...
genPoolParamPledge ::
  (MonadGen m, HasModelLedger era s, State.MonadState s m, MonadReader ModelGeneratorContext m) =>
...
genGenesesUTxOs ctx = do
  ...
  fmap Map.fromList $
    for (toList g') $ \(ModelValueSimple (x, _)) ->
      (\ui addr vl -> (ui, (addr, vl)))
        <$> freshUtxoId
        <*> freshPaymentAddress "gen"
        <*> pure x

-- TODO: what the delegates are doesn't really matter, and there *must* be at
-- least quorum of them for the ledger to work; so this should generate that
-- many genesis keys, plus some extras.
