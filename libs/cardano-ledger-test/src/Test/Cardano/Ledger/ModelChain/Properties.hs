...
import Test.Cardano.Ledger.ModelChain.Fixup (FixupValuesFlags (..), fixupValues)
...
propertyShrinkingImpl callback originalModel = flip execPropertyWriter () $ do
  discardModel <- case discardUnnecessaryTxnsImpl testGlobals originalModel of
    ...
    Just discardImplModel -> do
      ...
      let (discardErrs, discardModel) = fixupValues (FixupValuesFlags True) testGlobals discardImplModel
      ...
      tellProperty $ cover 90 (isNothing discardErrs) "no fixup errors on discard"
      tellProperty $ cover 55 (numAfter < numBefore) "numAfter < numBefore"

      case discardUnnecessaryTxnsWithErrors testGlobals discardModel of
        Nothing -> pure ()
        Just (discard2Errs, discard2Model) -> do
          tellProperty $ counterexample $ "discard2Model: " <> show discardImplModel
          let numAfter' = (lengthOf (_2 . traverse . modelTxs)) discard2Model
          tellProperty $ cover 90 (isNothing discard2Errs) "no fixup errors on discard2"
          tellProperty $ cover 55 (numAfter' == numAfter) "numAfter' == numAfter"

      pure discardModel

  (collapseModel, didCollapse) <- case collapseTransactionsWithErrors testGlobals discardModel of
    Nothing -> do
      pure (discardModel, False)
    Just (collapseErrs, collapseModel) -> do
      tellProperty $ counterexample $ "collapseModel: " <> show collapseModel
      tellProperty $ cover 90 (isNothing collapseErrs) "no fixup errors on collapse"
