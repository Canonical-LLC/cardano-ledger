...
ppPoolRewardInfo (PoolRewardInfo relStake pot params blocks lreward) =
  ppRecord
    ...
    [ ("poolRelativeStake", ppStakeShare relStake),
      ...
      ("leaderReward", ppReward lreward)
    ]

ppFreeVars :: FreeVars crypto -> PDoc
ppFreeVars (FreeVars ds addrs total mv pri) =
  ppRecord
    "FreeVars"
...
ppAns (RewardAns x) =
  ppSexp' mempty [ppMap ppCredential (ppSet ppReward) x]
...
ppRewardPulser (RSLP n free items ans) =
  ppSexp
    ...
    [ ppInt n,
      ppFreeVars free,
      ppMap ppCredential ppCoin items,
      ppAns ans
    ]
