...
class
    ...
    Except.runExceptT $ do
      (txBodyArguments, EraElaboratorTxAccum scripts dats) <- CPS.runWriterT $ do
        let liftEvalModelValue ::
              ...
              CPS.WriterT (EraElaboratorTxAccum era) (State.State (NewEpochState era, EraElaboratorState era)) (Either e a) ->
              CPS.WriterT
  ...
  elaborateBlocksMade globals mblocksMade = State.runState $ do
    ...
    compareModelLedger (cmsError mblocksMade)
