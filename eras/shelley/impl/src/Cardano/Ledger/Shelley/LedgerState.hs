...
startStep slotsPerEpoch b@(BlocksMade b') es@(EpochState acnt ss ls pr _ nm) maxSupply asc secparam =
  let SnapShot stake' delegs' poolParams = _pstakeGo ss
      f, stakeSize, k :: Rational
      stakeSize = fromIntegral (Map.size $ unStake stake')
      ...
      pulseSize = max 1 (ceiling ((stakeSize * f) / (4 * k)))
      ...
      mkPoolRewardInfoCurry =
        mkPoolRewardInfo
          (getField @"_d" pr, getField @"_a0" pr, getField @"_nOpt" pr)
          _R
          b
      ...
      allPoolInfo = Map.map mkPoolRewardInfoCurry poolParams
      ...
      blockProducingPoolInfo = Map.mapMaybe rightToMaybe allPoolInfo
      ...
      genRewardAcnt = getRwdCred . _poolRAcnt . poolPs
      collectLRs acc poolRI =
        if genRewardAcnt poolRI `Map.member` _rewards ds
          then
            Map.insertWith
              Set.union
              (genRewardAcnt poolRI)
              (Set.singleton $ poolLeaderReward poolRI)
              acc
          else acc
      ...
      -- The data in 'FreeVals' to supply individual stake pool members with
...
completeRupd
  ( Pulsing
    ...
    let rs' = Map.unionWith Set.union rs_ lrewards
